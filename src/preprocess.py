...
UNIT_BACKUP = "data/processed/cleaned_data_with_unit.csv"
...
if new_data:
    ...
    if os.path.exists(UNIT_BACKUP):
        existing = pd.read_csv(UNIT_BACKUP)
    ...
    combined          = combined.drop(columns=["max_cycle"])

    combined.to_csv(UNIT_BACKUP, index=False)

    combined.drop(columns=["unit"]).to_csv(OUTPUT_FILE, index=False)

    with open(LOG_FILE, "a") as f:
        for file in new_processed_files:
            f.write(file + "\\n")

    print(f"[SUCCESS] Total rows: {len(combined)}")
else:
